...
def apply_discount(price:float, discount_tier:str) -> float:
    ...
    return round(price * (100 - discounts.get(discount_tier.lower(), 0))/100, 2)
    ...
    messages = [
        SystemMessage(
            content= (
                ...
                "Pass the exact price returned — do NOT pass a made-up number.\n"
                "3. NEVER calculate discounts yourself using math.\n"
                "Always use the apply_discount tool.\n"
                "4. If the user does not specify a discount tier, \n" 
                "ask them what to use — do NOT assume a default."
                )
            ),
